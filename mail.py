...
def send_results_email(results, query, today, last_week, config):
    ...
    def format_email_content(results, query, today, last_week, config):
        ...
        <head>
            ...
            <style>
                /* Force full width and prevent mobile text-size adjustments */
                html, body {{ width:100% !important; -webkit-text-size-adjust:100%; -ms-text-size-adjust:100%; margin:0; padding:0; }}
                body {{ font-family: Arial, sans-serif; margin: 0; padding: 20px; box-sizing: border-box; }}
                /* Constrain email content and center it */
                .container {{ max-width: 640px; width: 100%; margin: 0 auto; box-sizing: border-box; }}
                h1 {{ color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px; margin-top:0; }}
                h2 {{ color: #34495e; margin-top: 30px; }}
                .paper {{ margin-bottom: 30px; padding: 15px; border: 1px solid #ecf0f1; border-radius: 5px; box-sizing: border-box; word-wrap: break-word; overflow-wrap: break-word; }}
                ...
                .query-info {{ background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 20px; box-sizing: border-box; word-break: break-word; }}
                .footer {{ margin-top: 30px; padding-top: 15px; border-top: 1px solid #ecf0f1; font-size: 12px; color: #7f8c8d; word-break: break-word; }}
    ...
    def format_no_llm_email_content(papers_with_abstracts, query, today, last_week, paper_count, max_papers_for_llm, config):
        ...
        <head>
            ...
            <style>
                ...
                .paper-title {{ margin: 10px 0; padding: 8px; background-color: #ecf0f1; border-radius: 3px; word-break: break-word; }}
